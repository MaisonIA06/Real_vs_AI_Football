...
export default function QuizJoinPage() {
  ...
  // Vérifier l'existence de la room (fetch natif, fiable sur iOS).
  useEffect(() => {
    if (roomCode.length === 6) {
      const verifyRoom = async () => {
        const apiUrl = `${window.location.protocol}//${window.location.host}/api/quiz/rooms/${roomCode.toUpperCase()}/`;
        try {
  ...
  const handleJoin = (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    setError(null);
    navigate(`/quiz/play/${roomCode.toUpperCase()}`, { state: { pseudo } });
  };
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-4 py-12">
      ...
      >
        ...
        >
          <span className="gradient-text">Quiz Foot ⚽</span>
        </motion.h1>
